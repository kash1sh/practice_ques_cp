...
void modify(int x)
{
    ...
    auto it = dif.find(x);
    if (it != end(dif))
    { // x is currently present in dif, remove it
    ...
    { // x is not currently in dif, insert it
        dif.insert(x);
        auto iit = dif.find(x); // insert x into dif
        // it = iterator corresponding to x
        int a = *prev(iit), b = *next(iit);
