...
void solve()
{
    ...
    sort(v.begin(), v.end());
    ll mini = INT_MAX;
    ll ans = 0, res = INT_MIN, prev = INT_MAX;
    for (ll i = n - 1; i >= max(0ll, n - 200ll); i--)
    {
        for (ll j = i - 1; j >= max(n - 200ll, 0ll); j--)
        {
            ll v1 = (i + 1) * (j + 1);
            ll orv = (a[i] | a[j]);
            // if (orv > prev)
        ...
        // ll v1 = (v[i].ss) * (v[i + 1].ss);
        // ll orv = (v[i].ff | v[i + 1].ff);
        // ans = v1 - (k * orv);
        // res = max(res, ans);
    }
    cout << res << endl;
